    ...
    describe('onlogin', () => {
      ...
      describe('resolve', () => {
        it('resolveされること', done => {
          ...
          expect(onloginStub.called).to.equal(false)              // まだresolveされないこと
          expect(loginForm.vm.error).to.equal('')                 // エラーメッセージは初期化されること
          expect(loginForm.vm.disableLoginAction).to.equal(true)  // ログインアクションは不可

          // 状態の反映
          loginForm.vm.$nextTick(() => {
            expect(onloginStub.called).to.equal(true)             // resolveされる
        ...
        it('rejectされること', done => {
          ...
          loginForm.vm.$nextTick(() => {
            ...
            loginForm.vm.$nextTick(() => {
              ...
              expect(loginForm.vm.disableLoginAction).to.equal(false)  // ログインアクションは不可
              done()
            })
          })
        })
